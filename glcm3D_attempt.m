...
OPT.quantLevel = 16;        % number of gray level bins 
...
for i=1:length(filenames)
    ...
    if (exist(string(glcmFile), 'file') == 2)
        ...
        GLCMS = CreateGLCM_Local(volimg,OPT.quantLevel,[0 1],OPT.D,OPT.NeighborSize,mask); 
        save(glcmFile,'GLCMS');
    end
    ...
    for prop=1:length(OPT.glcm_properties)
        ...
        for z = 1:length(img(1,1,:))
            if z ==1
                imwrite(uint16(vol.img(:,:,z)),fullfile(outpath,join(TextureFileName)));
            else
                imwrite(uint16(vol.img(:,:,z)),fullfile(outpath,join(TextureFileName)),'WriteMode','append')
